...
class AddFootballGameViewModel: ViewModel() {
    ...
    private fun fetchTeams() {
        coroutineScope.launch {
            ...
            try {
                ...
                Log.i("AddFootballGameViewModel", fetchTeamStatus.toString());
            }
        }
    }
    ...
    fun addFootballGame(){
        coroutineScope.launch {
            val getFootballGameDeferred = API.retrofitService.addFootballGame( PostFootballGame(teamAId, teamBId) )
            try {
                val result = getFootballGameDeferred.await()
                _message.value = "game added successfully";
                createGameDetails(result.id)
                Log.i("AddFootballGameViewModel", result.toString());
            }catch (t: Throwable){
                _message.value = "error: " + t.toString();
                Log.i("AddFootballGameViewModel", message.value!!);
            }
        }
    }
    ...
    private fun createGameDetails(eventId: String){
        coroutineScope.launch {
            val getGameInfoDeferred = API.retrofitService.createGameDetails(PostGameInfo("Football", "", "2021-01-01T00:00:00.000Z","", "", "", eventId, ""));
            try {
                ...
                _message.value = "error: " + t.toString()
                Log.i("AddFootballGameViewModel. hereeeeeeeeee: ", message.value!!);
            }
        }
    }
