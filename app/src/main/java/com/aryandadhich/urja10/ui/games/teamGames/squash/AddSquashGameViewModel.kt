...
class AddSquashGameViewModel: ViewModel() {
    ...
    private fun fetchTeams() {
        coroutineScope.launch {
            ...
            try {
                ...
                Log.i("AddSquashGameViewModel", fetchTeamStatus.toString());
            }
        }
    }
    ...
    fun addSquashGame(){
        coroutineScope.launch {
            val getSquashGameDeferred = API.retrofitService.addSquashGame( PostSquashGame(teamAId, teamBId) )
            try {
                val result = getSquashGameDeferred.await()
                _message.value = "game added successfully";
                createGameDetails(result.id)
                Log.i("AddSquashGameViewModel", result.toString());
            }catch (t: Throwable){
                _message.value = "error: " + t.toString();
                Log.i("AddSquashGameViewModel", message.value!!);
            }
        }
    }
    ...
    private fun createGameDetails(eventId: String){
        ...
        Log.i("AddSquashGameViewModel", "here i am");
        coroutineScope.launch {
            val getGameInfoDeferred = API.retrofitService.createGameDetails(PostGameInfo("Squash", "", "2021-01-01T00:00:00.000Z","", "", "", eventId, ""));
            try {
                ...
                _message.value = "error: " + t.toString()
                Log.i("AddSquashGameViewModel. hereeeeeeeeee: ", message.value!!);
            }
        }
    }
