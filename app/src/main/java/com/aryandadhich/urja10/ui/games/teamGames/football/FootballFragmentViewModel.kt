package com.aryandadhich.urja10.ui.games.teamGames.football

import android.util.Log
import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import com.aryandadhich.urja10.network.API
import com.aryandadhich.urja10.ui.games.common.gameInfo.GetGameInfo
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.Job
import kotlinx.coroutines.launch

class FootballFragmentViewModel: ViewModel() {
    private var _footballGames= MutableLiveData<List<FootballGame>?>();
    val footballGames: LiveData<List<FootballGame>?>
        get() = _footballGames;

    private var _status = MutableLiveData<String>()
    val status: LiveData<String>
        get() = _status;

    private var _loadData = MutableLiveData<Boolean>()
    val loadData: LiveData<Boolean>
        get() = _loadData;

    private val viewModelJob = Job();
    private val coroutineScope = CoroutineScope(viewModelJob + Dispatchers.Main);
    init {
        fetchFootballGames();
    }

    private fun fetchFootballGames() {
        coroutineScope.launch {
            val getFootballDeferred = API.retrofitService.getAllFootballGames();
            try {
                val result = getFootballDeferred.await()

                for(game in result){
                    val gettingTeam1data = API.retrofitService.getTeamById(game.teamA)
                    val gettingTeam2data = API.retrofitService.getTeamById(game.teamB)

                    try {
                        val teamA = gettingTeam1data.await()
                        Log.i("FootballFragmentViewModel: ", "yaha bhi nai pohocha??" + teamA.toString());

                        val teamB = gettingTeam2data.await()
                        if(teamB != null){
                            game.teamB = teamB.houseName;
                        }
                        if (teamA != null) {
                            game.teamA = teamA.houseName
                        };

                    }catch (t: Throwable){
                        _status.value = t.toString();
                        Log.i("FootballFragmentViewModel", t.toString());
                    }

                    try {
                        val teamB = gettingTeam2data.await()
                        if(teamB != null){
                            game.teamB = teamB.houseName;
                        }
                    }catch (t: Throwable){
                        _status.value = t.toString();
                        Log.i("FootballFragmentViewModel", t.toString());
                    }

                    if(game.id == result[result.size - 1].id){
                        _footballGames.value = result;
                        _status.value = "fetch successfully";
                    }


                    Log.i("FootballFragmentViewModel in here at: ", game.toString());
                }


                Log.i("FootballFragmentViewModel", _footballGames.value.toString());
                _loadData.value = true;
            }catch (t: Throwable){
                _status.value = t.toString();
                Log.i("FootballFragmentViewModel. yaha wali: ", t.toString());
            }
        }
    }
}