...
class AddChessGameFragmentViewModel: ViewModel() {
    ...
    fun addChessGame(){
        coroutineScope.launch {
            val getChessGameDeferred = API.retrofitService.addChessGame( PostChessGame(playerA, playerB, winner) )
            try {
                val result = getChessGameDeferred.await()
                _message.value = "game added successfully";
                createGameDetails(result.id)
                Log.i("AddChessGameViewModel", result.toString());
            }catch (t: Throwable){
                _message.value = "error: " + t.toString();
                Log.i("AddChessGameViewModel here", message.value!!);
            }
        }
    }
    ...
    private fun createGameDetails(eventId: String){
        ...
        Log.i("AddChessGameViewModel", "here i am");
        coroutineScope.launch {
            val getGameInfoDeferred = API.retrofitService.createGameDetails(PostGameInfo("Chess", "", "2021-01-01T00:00:00.000Z","", "", "", eventId, ""));
            try {
                ...
                _message.value = "error: " + t.toString()
                Log.i("AddChessGameViewModel. hereeeeeeeeee: ", message.value!!);
            }
        }
    }
